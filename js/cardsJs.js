...
//calculates the persentage of attendance of eache event , return eache event with its percentage, id, and capacity
const calculatePercentageAssistance = (events)=>{
  const percentages = events.map(event =>{
    ...
    return {'percentage': percentage, 'eventId': event._id, 'capacity': Number(event.capacity)}
  })
